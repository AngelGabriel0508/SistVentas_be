...
class _ProductModalPageState extends State<ProductModalPage> {
  ...
  String? _validateCode(String? value) {
    ...
    if (!value.startsWith('P')) {
      return 'El código debe comenzar con P';
    }

    final String afterP = value.substring(1);
    final RegExp numericRegex = RegExp(r'^\d+$');
    if (!numericRegex.hasMatch(afterP)) {
      return 'Ingrese números después de la P';
    }

    if (value.length != 4) {
      return 'El código debe contener 4 dígitos';
    }
    if (_codeExists) {
      return 'El codigo ya está registrado';
    }
  ...
  void _checkCodeExists(String value) async {
    try {
      bool codeExists = await ApiServiceProduct.checkExistingCode(value);
      setState(() {
